...
import { editColorSchema } from "@/schemas";
import { editColor } from "@/actions/color.actions";

interface EditColorFormProps {
  color: Color;
}

export const EditColorForm = ({ color }: EditColorFormProps) => {
  ...
  const onSubmit = (values: z.infer<typeof editColorSchema>) => {
    setError("");

    startTransition(() => {
      editColor(color.id, values).then((data) => {
        if (data?.error) {
          setError(data.error);
        } else {
